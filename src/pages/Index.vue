<template>
  <q-page class="full-width column wrap justify-start items-center content-center q-pt-md">
    ...
    <p class="text-body2 text-justify">Это удобное мобильное приложение "2 in 1", в котором специалист по недвижимости может моментально рассчитать стоимость ипотечного полиса для клиента и свою комиссию.</p><p class="text-body2 text-justify"> Промокод для пользования доступен при подписке в <a class="insta-link" href="https://www.instagram.com/khramov_insur/" target="blank">Instagram</a></p>
    <q-form class="">
      <q-select
        @input="calc" 
        v-model="bank" 
    ...
    <div v-if="results.length" style="display: contents" class="items-center content-center">
    <q-separator />
    ...
    <q-markup-table>
      ...
      <tbody>
        <tr v-for="result in results" :key="result.id">
          <td>{{result.label}}</td>
          <td>
            <p class="q-mb-none">Жизнь: {{result.life}}</p>
            <p class="q-mb-none">Имущество: {{result.estate}}</p>
            <p class="q-mb-none">Титул: {{result.title}}</p>
    ...
    <p class="text-body2 text-justify q-mt-md">Вы можете скопировать результаты расчета для клиента по каждой страховой кликнув на иконку "Копировать"</p>
    <q-separator inset/>
    ...
    <p class="text-body2 text-justify q-mt-md">В нашей партнерской программе работает простая формула округления комиссии до сотен рублей в большую сторону, и это всегда не менее 1000 руб. Никаких вычетов налогов. Ровно 50% от комиссии страховой компании.</p>
    <q-btn 
      ...
      <p class="text-body2 text-justify q-mt-md">Вы можете скопировать (в буфер) для себя и своего клиента список документов, которые необходимы для оформления полисов в зависимости от выбранных рисков.</p>
    <div style="display: none">{{listText}}</div>
    <h6 class="text-h5">Посчитали, что дальше?</h6>
    <p class="text-body2 text-justify q-mt-md">Это предварительный расчет, который можно озвучить клиенту. Однако, у страховых часто меняются тарифы, мы это, конечно, отслеживаем и актуализируем коэффициенты. Но точный расчет производится при оформлении, поэтому <a class="text-orange-5" href="https://wa.me/79219532923">пишите</a> Антону Храмову, Вашему страховому брокеру. Или звоните <a class="text-orange-5" href="tel:+79219532923">+7 (921) 953-29-23</a></p>
      <div class="q-mt-md text-center">
      <a class="text-orange-5" target="blank" href="https://taplink.cc/khramov_insur">Ссылки на контакты и сайты</a>
      </div>
      <div class="q-mt-md text-center">
      <a class="insta-link" target="blank" href="https://www.instagram.com/khramov_insur/">Instagram</a>
      </div>
    <p class="text-body2 text-justify q-mt-md">P.S. За рекомендацию коллегам при оформлении полиса для любого банка Вы получаете 1000 руб.</p>
    </div>
    <q-tooltip v-model="copyBanner">Текст скопирован</q-tooltip>
  </q-page>
</template>
...
export default {
  ...
  methods: {
    ...
    calcComission(cost){
      let comission = Math.ceil(Math.ceil((cost * this.bank.comission) / 2) / 100) * 100;
      comission >= 1000 ? comission : comission = 1000
      return comission
    },
    calc() {
      this.results = []
      ...
      for (let company of this.INS_COMPANIES){
        ...
        result.text += `Ваша комиссия составит ${resultSum}р.: ((${resArr.join('+')}) × ${this.setPercentage(this.bank.comission)}) ÷ 2 = ${resultSum}р.`
        result.text += ``
        result.clientText = `${company.label}. Страховая сумма для ${this.bank.label} ${this.setDebt} руб. Стоимость полиса: ((${resArr.join('+')})=${this.calcSum(resArr)} руб.`
